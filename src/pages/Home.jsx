  ...
  useEffect(() => {
    ...
    function updateDateOutput(value) {
      ...
      if (value === "weekly") {
        ...
        setDateOutput(
          `${prevWeek.getDate()} ${months[prevWeek.getMonth()]} - ${day} ${
            months[month]
          }`
        );
      } else if (value === "daily") {
        setDateOutput(`${day} ${months[month]} ${year}`);
    ...
    if (scheduleSelect) {
      ...
      scheduleSelect.onchange = (e) => updateDateOutput(e.target.value);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const chartData = {
      ...
      monthly: {
        ...
        data: [20, 25, 30, 22, 28, 35, 40, 33, 29, 31, 27, 26],
      },
    };

    let reportsChart = null;
    let crimePieChart = null;
    ...
    if (ctx) {
      ...
      reportsChart = new Chart(ctx, {
        ...
        options: {
          responsive: true,
          plugins: {
            legend: { labels: { color: getColors().fontColor } },
            tooltip: {
              backgroundColor: getColors().secondryColor,
              titleColor: getColors().fontColor,
              bodyColor: getColors().fontColor,
            },
          },
          scales: {
            y: { ticks: { color: getColors().fontColor } },
            x: { ticks: { color: getColors().fontColor } },
          },
        },
      });
    ...
    const crimeCtx = document.getElementById("crimePieChart");
    if (crimeCtx) {
      if (Chart.getChart(crimeCtx)) Chart.getChart(crimeCtx).destroy();

      crimePieChart = new Chart(crimeCtx, {
        type: "pie",
        data: {
          labels: [
            "سرقة",
        ...
        options: {
          plugins: {
            legend: {
              position: "right",
              labels: { color: getColors().fontColor },
            },
            tooltip: {
              backgroundColor: getColors().secondryColor,
              titleColor: getColors().fontColor,
    ...
    return () => {
      ...
      if (crimePieChart) crimePieChart.destroy();
    };
  }, []);
    ...
    <div className="wrapperr">
      <div className="dashboard-content row align-items-center">
        <div className="info col-12 col-lg-6">
          <h2>لوحة التحكم</h2>
          <p>تابع البلاغات والإحصائيات وإدارة النظام بسهولة.</p>
        </div>

        <div className="segment-container col-12 col-lg-6 d-flex flex-wrap justify-content-lg-end gap-2">
          <div className="schedule-picker segment-item">
            <div className="calendar-icon d-flex align-items-center justify-content-center">
              <i className="fa-regular fa-calendar-days ms-2"></i>
              <div className="output ms-2">{dateOutput}</div>
            </div>
          ...
          <div className="crime-selector segment-item">
            <select id="crimeType" defaultValue="all">
              ...
              <option value="vandalism">تخريب</option>
              <option value="cybercrime">جريمة إلكترونية</option>
              <option value="harassment">تحرش</option>
              <option value="drug">مخدرات</option>
              <option value="other">أخرى</option>
            </select>
          </div>

          <a href="#" className="segment-item">
            AI assistant
          </a>
        </div>
      </div>

      <div className="reborts mt-2 row g-3">
        {[
          {
            title: "البلاغات الكلية",
            value: "12600",
            note: "↝ +2% آخر 4 شهور",
          },
          { title: "البلاغات الجديدة", value: "600", note: "↝ +2% آخر يوم" },
          { title: "قيد المراجعة", value: "12600", note: "↝ +2% آخر 4 شهور" },
          { title: "البلاغات الحرجة", value: "60", note: "↝ +5% آخر يوم" },
        ].map((r, i) => (
          <div key={i} className="col-lg-3 col-md-6">
            <div className="rebort-box">
              <p>{r.title}</p>
              <h4 className="fw-bold">{r.value}</h4>
              <p>{r.note}</p>
            </div>
          </div>
        ))}
      ...
      <div className="row charts mt-2">
        ...
        <div className="pie-chart col-lg-6">
      ...
      >
        <div className="map-info">
          <h2>خريطة توزيع البلاغات في مصر</h2>
          <p>
            الخريطة بتوضح المحافظات اللي تم تسجيل بلاغات فيها بشكل أوضح، بينما
            المحافظات اللي باللون الباهت مفيهاش بلاغات مسجلة. الهدف من الخريطة
            إنك تقدر تشوف بسرعة أماكن تركز البلاغات على مستوى الجمهورية.
          </p>{" "}
        </div>
        <div className="map">
          <svg
            ...
            viewBox="0 0 548.58221 498.86664"
